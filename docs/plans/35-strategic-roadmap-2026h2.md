...
Status: **Strategy / sequencing.** **Wave 0 (i18n), Wave A (A1/A2/A3 —
Plans 36/37/38), Wave B1 (Plans 29-D + 29-D-A), Wave B2 (Plan 39), and
Plan 40 (Composer Redesign) shipped by 2026-06-06. Wave C1 (multi-user
account layer + user-scoped DB) shipped 2026-06-11** (Holzi PRs #84/#85 +
holzi-monorepo #3; backend CI green). C1 plan files:
`docs/plans/2026-06-11-wave-c1-multi-user-{design,impl}.md` in the Holzi
backend repo.
Next executable slice: **Wave C2** (email magic-link login + family admin /
invites / roles / sharing) — outline at
`docs/plans/2026-06-11-wave-c2-family-admin-outline.md` (Holzi repo).
...
**Status**: **C1 completed 2026-06-11** (account layer + user-scoped DB; see
the C1 design/impl docs in the Holzi repo). **C2 + C3 outlined, not started.**
Note: C1 chose **session tokens + email magic-link** (no `password_hash`) and
a pluggable `IdentityResolver` (DID-ready for haex-vault) — the C1 column list
below is superseded by the C1 design doc. C2/C3 outlines now also carry the
multi-user gaps C1 deferred (agent tool-loop identity, per-user
`channel_prompts`, global approvals, FK-on-upgraded-DBs).
